...
export async function insertRider(data) {
  ...
  try {
    ...
    warn(
      `Master (Node 1) failed: ${err.message}. Falling back to ${fragment.name}...`
    );
...
export async function updateRider(id, data) {
  ...
  try {
    ...
    if (!fragment)
      throw new Error(
        "Master down and Courier not provided for routing fallback."
      );
    ...
    return await runTransaction(fragment.pool, async (conn) => {
      ...
      if (!rows.length)
        throw new Error(`Rider ${id} not found on ${fragment.name}`);
      const oldRow = rows[0];
...
export async function getAllRiders() {
  try {
    const [rows] = await nodes.node1.pool.query(
      "SELECT * FROM Riders ORDER BY id DESC"
    );
    return rows;
  } catch (err) {
    warn("Master read failed. Merging fragments...", err.message);
    try {
      const [r2] = await nodes.node2.pool.query(
        "SELECT * FROM Riders ORDER BY id DESC"
      );
      const [r3] = await nodes.node3.pool.query(
        "SELECT * FROM Riders ORDER BY id DESC"
      );
      // Merge and sort
